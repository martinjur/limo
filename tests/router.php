...
test_case("Router");
test_case_describe("Testing limo router functions.");
...
{
    assert_trigger_error('route_build', ['UNKOWN', '/', 'aaa']);
    ...
    $r = route_build("GET", ["/test/*/two", ["first"]], 'get_index');
    assert_match($r["pattern"], "/test/truc/two");
    assert_match($r["pattern"], "/test/truc/two/");
    ...
    assert_match($r["pattern"], "/limo.jpg");
    assert_match($r["pattern"], "/limo.jpg/");
    assert_match($r["pattern"], "/limo.jpg/thumb");

    /* testing a complex route and parameters names*/
    $path = "/test/:my/*/complex/**/:route";
    $params = ["mmy", "second", "lazy", null];

    $r = route_build("GET", [$path, $params], 'get_index');
    assert_match($r["pattern"], "/test/my/first/complex/very-big/route/69");
    assert_no_match($r["pattern"], "/test/truc/2");
...
function test_router_route()
{
    ...
    $r = route("post", "/blog/:id", "my_post_func", ['params' => ['extra' => 10]]);
    ...
    $r = route("get", "/blog/:id", "my_get_func", ['params' => ['id' => 10]]);
    assert_match($r[2]["pattern"], "/blog/102");
}
...
function test_router_find_route()
{
    ...
    $r = route_find("GET", "/limo.jpg/thumb");

    assert_equal($r["callback"], "my_jpeg");
    assert_equal($r["params"][0], "limo");
    ...
    $extra_p = [123, 'id' => 123, 'name' => 'abc'];

    route("get", "/no/cat/:id", "my_p_func");
    route("get", "/with/cat/:id", "my_p_func", ['params' => $extra_p]);
    $routes = route("get", "/indexed/cat/*", "my_p_func", ['params' => $extra_p]);
