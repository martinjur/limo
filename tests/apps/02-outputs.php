...
{
    return render("Lorem %s dolor sit amet.", null, ['ipsum']);
}
...
function partial_example()
{
    ...
    return partial('no layout there %s', ['buddy']);
}

dispatch('/text', 'text_file');
function text_file()
{
    return render_file(dirname(__FILE__, 2) . '/data/empty_text_file.txt');
}

dispatch('/jpeg', 'jpeg_file');
function jpeg_file()
{
    return render_file(dirname(__FILE__, 2) . '/data/deer.jpg');
}
