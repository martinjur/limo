...
test_case("HTTP");
test_case_describe("Testing limo HTTP utils functions.");
...
function test_http_ua_accepts()
{
    ...
    $response = test_request(TESTS_DOC_ROOT . '05-content_negociation.php', 'GET', false, [], ["Accept: image/png"]);
    assert_equal("Oops", $response);

    $response = test_request(TESTS_DOC_ROOT . '05-content_negociation.php', 'GET', false, [], ["Accept: text/html"]);
    assert_equal("<h1>HTML</h1>", $response);

    $response = test_request(TESTS_DOC_ROOT . '05-content_negociation.php', 'GET', false, [], ["Accept: application/json"]);
...
function test_http_redirect()
{
    ...
    $response = test_request($url . '&key1=value1');
    assert_equal($response, '/redirected&key1=value1');

    $response = test_request($url . '&key1=value1&key2=value2');
