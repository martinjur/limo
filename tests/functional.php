...
function test_functional_request()
{
    ...
    assert_header($response, 'X-Limo', LIM_NAME);
}

function test_functional_session()
{
    $response = test_request(TESTS_DOC_ROOT . '06-session.php/');
    ...
    assert_equal($response, "LIMO" . str_replace('.', 'x', LIMO));
}

function test_functional_routing()
{
    $path = TESTS_DOC_ROOT . '03-routing.php/';
    $response = test_request($path . 'route0');
    assert_equal($response, 'route0');
    $response = test_request($path . 'route1');
    assert_equal($response, 'route1');
    $response = test_request($path . 'route2');
    assert_equal($response, 10);
    $response = test_request($path . 'route3');
    assert_equal($response, 20);
    $response = test_request($path . 'route4');
    assert_equal($response, 20);

    if (version_compare(PHP_VERSION, '5.3.0') >= 0) {
        $response = test_request($path . 'route-lambda');
        assert_equal($response, 'LAMBDA CALL');
    }

    $response = test_request($path . 'route5');
    assert_equal($response, 'human');
    $response = test_request($path . 'route5b');
    assert_equal($response, 'human');
    $response = test_request($path . 'route6');
    assert_equal($response, 'human');
    $response = test_request($path . 'route6b');
    assert_equal($response, 'human10');
    $response = test_request($path . 'route6c');
    assert_equal($response, 'human10');

    $response = test_request($path . 'route7/123');
    assert_equal($response, 123);
    $response = test_request($path . 'route7b/123');
    assert_equal($response, 123);
    $response = test_request($path . 'route7c/123');
    assert_equal($response, 1230);
    $response = test_request($path . 'route7d/123');
    assert_equal($response, 1230);
    $response = test_request($path . 'route7e/123');
    assert_equal($response, 2460);
    $response = test_request($path . 'route7f/123');
    assert_equal($response, 2460);
    $response = test_request($path . 'route7g');
    assert_equal($response, 200);
    $response = test_request($path . 'route7h');
    assert_equal($response, 200);
    $response = test_request($path . 'route8/123');
    assert_equal($response, 123);
    $response = test_request($path . 'route8b/123');
    assert_equal($response, 123);
    $response = test_request($path . 'route8c/10');
    assert_equal($response, 5);
    $response = test_request($path . 'route9/123');
    assert_equal($response, 2460);
    $response = test_request($path . 'route9b/123');
    assert_equal($response, 2460);
    $response = test_request($path . 'route10/123');
    assert_equal($response, 2460);
    $response = test_request($path . 'route10b/123');
    assert_equal($response, 2460);

    /* http methods dispatching */
    $response = test_request($path . 'route11');
    ...
    /* undefined route */
    $response = test_request($path . 'unknown_route');
...
function test_functional_params()
{
    ...
    $response = test_request($path);
    assert_equal($response, 'HELLO');

    $response = test_request($path . '?/&sort=asc');
    assert_match('/sort=asc/', $response);

    $response = test_request($path . '/books/fr');
    assert_equal($response, 'lang=fr');

    $response = test_request($path . '?uri=books/fr&sort=asc&page=2');
    assert_match('/sort=asc/', $response);
    assert_match('/page=2/', $response);

    $post = json_encode(['title' => 'hello world']);
    $headers = [
