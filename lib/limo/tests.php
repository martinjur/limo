...
if (!array_key_exists("limo", $GLOBALS)) {
    $GLOBALS["limo"] = [];
}
if (!array_key_exists("test_cases", $GLOBALS["limo"])) {
    $GLOBALS["limo"]["test_cases"] = [];
}
if (!array_key_exists("test_errors", $GLOBALS["limo"])) {
    $GLOBALS["limo"]["test_errors"] = [];
}
if (!array_key_exists("test_case_current", $GLOBALS["limo"])) {
    $GLOBALS["limo"]["test_case_current"] = null;
}
if (!array_key_exists("test_suites", $GLOBALS["limo"])) {
    $GLOBALS["limo"]["test_suites"] = null;
}
...
{
    $name = $GLOBALS["limo"]["test_suites"];
    $failures = 0;
    $tests = 0;
    $passed_tests = 0;
    $assertions = 0;

    foreach ($GLOBALS["limo"]["test_cases"] as $test) {
...
function test_case($name)
{
    ...
    if (!array_key_exists($name, $GLOBALS["limo"]["test_cases"])) {
        $GLOBALS["limo"]["test_cases"][$name] = [
            "name"        => $name,
            "assertions"  => 0,
            "failures"    => 0,
            "description" => null
        ];
        $GLOBALS["limo"]["test_case_current"] = $name;
    } else {
    }
}
...
{
    $name = $GLOBALS["limo"]["test_case_current"];
    ...
    if (!is_null($name)) {
        $test = $GLOBALS["limo"]["test_cases"][$name];
...
function test_case_all_func()
{
    ...
    while ($func = array_shift($functions)) {
        $regexp = "/^test_{$name}_(.*)$/";
        if (!preg_match($regexp, $func)) {
            continue;
        }

        if ($func == test_before_func_name()) {
            continue;
        }
...
function &test_case_current()
{
    ...
    return $GLOBALS["limo"]["test_cases"][$name];
}
...
function test_before_func_name()
{
    ...
    return "before_each_test_in_" . $test["name"];
}
...
function test_before_assert_func_name()
{
    ...
    return "before_each_assert_in_$name" . $test["name"];
}

function test_run_assertion()
{
    $name = $GLOBALS["limo"]["test_case_current"];
    $GLOBALS["limo"]["test_cases"][$name]['assertions']++;
...
function test_error_handler($errno, $errstr, $errfile, $errline)
{
    ...
    $GLOBALS["limo"]["test_errors"][] = [$errno, $errstr, $errfile, $errline];
...
function test_assert_failure($script, $line, $message)
{
    ...
    $name = $GLOBALS["limo"]["test_case_current"];
    $GLOBALS["limo"]["test_cases"][$name]['failures']++;
}
 ...
 * @author Nando Vieira
 */
function test_request($url, $method = "GET", $include_header = false, $post_data = [], $http_header = [])
{
